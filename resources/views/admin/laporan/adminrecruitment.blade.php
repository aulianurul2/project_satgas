...
{{-- Contoh: $pendaftaranAktif = App\Models\Setting::where('key', 'pendaftaran_aktif')->value('value'); --}}
...
<div class="container mx-auto py-8 px-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900">Daftar Pelamar</h1>
            <p class="text-md text-gray-600 mt-1">Manajemen data pelamar, berkas, dan proses seleksi anggota baru.</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-md mb-8 border border-gray-200">
        ...
        {{-- Ganti URL action sesuai dengan route index tabel Anda. Gunakan GET method. --}}
        <form method="GET" action="{{ url()->current() }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            
            <div class="col-span-1">
                <label for="start_date" class="block text-sm font-medium text-gray-700 mb-1">Dari Tanggal</label>
                {{-- Gunakan old() atau parameter request() untuk mempertahankan nilai filter --}}
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    {{-- Berkas (CV, Essay, Foto) digabung menjadi satu kolom --}}
                    <td class="p-3 whitespace-nowrap text-center text-sm">
                        <a href="{{ asset('storage/' . $pelamar->cv) }}" target="_blank" class="text-blue-500 hover:text-blue-700 font-medium text-xs block">CV</a>
                        <a href="{{ asset('storage/' . $pelamar->essay) }}" target="_blank" class="text-blue-500 hover:text-blue-700 font-medium text-xs block">Essay</a>
                        <a href="{{ asset('storage/' . $pelamar->pas_foto) }}" target="_blank" class="text-blue-500 hover:text-blue-700 font-medium text-xs block">Foto</a>
                    </td>
                    <td class="p-3 whitespace-nowrap text-sm text-gray-500">
                        {{ $pelamar->created_at ? $pelamar->created_at->format('d M Y') : '-' }} {{-- Mengganti $pelamar->tanggal_pendaftaran dengan $pelamar->created_at --}}
                    </td>
    ...
    <div class="mt-6 flex justify-between items-center">
        ...
        <div>
            {{ $pelamars->links() }} {{-- Ini adalah kunci untuk menampilkan pagination Blade --}}
        </div>
    </div>
